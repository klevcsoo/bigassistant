...
// ---------- REQUESTS ----------
// export const getUserFromCookie = async (req: any): Promise<UserRecord | undefined> => {
...
//     } catch {
//         console.log('Invalid id token was given');
//         return;
//     }
// }

export const getUserFromCookie = async (req: any, res: any, onUser:(user: UserRecord) => void): Promise<void> => {
    if (!req.cookies || !req.cookies.__session) {
        console.log('No session token cookie found');
    ...
    try {
        const idToken = String(req.cookies.__session);
        const uid = (await admin.auth().verifyIdToken(idToken)).uid
        onUser(await admin.auth().getUser(uid)); return;
    } catch {
        console.log('Invalid id token was given');
        res.redirect('/login'); return;
    }
